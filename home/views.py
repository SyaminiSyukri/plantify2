...
def edit_plant(request, image_id):
    image = get_object_or_404(Image, id=image_id)

    if request.method == "POST":
        image.sname = request.POST.get("sname")
        image.ldesc = request.POST.get("ldesc")
        image.save()
        return redirect("index")

    return render(request, "home/edit_plant.html", {"image": image})
